...
class RASVSR(nn.Module):
   def __init__(self,
       ...
       if is_low_res_input:
           self.feat_extract = nn.Sequential(ConvResidualBlocks(3, mid_channels, 5),MultiScaleAttention(mid_channels))
           self.feat_extract_2 = nn.Sequential(ConvResidualBlocksModified(3, mid_channels, 5), MultiScaleAttention(mid_channels))
       else:
   ...
   def forward(self, lqs):
       ...
       if self.cpu_cache:
           feats['spatial'] = []
           for i in range(0, t):
               feat1 = self.feat_extract(lqs[:, i, :, :, :]).cpu()
               #
               feat2 = self.feat_extract_2(lqs[:, i, :, :, :]).cpu()
               feat = feat1 + feat2
               #
